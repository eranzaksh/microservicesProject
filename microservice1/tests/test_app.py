...
def test_invalid_timestream_request(client):
    # Test request with invalid data.
    test_data = {
        "token": "test-token",
        "data": {
            "email_timestream": "invalid_timestamp",
            "other_email_data": "test"
        }
    }
    response = client.post('/send-email', 
                         json=test_data,
...
def test_timestream_validation(client):
    ...
    # Test with timestamp too far in future
    test_data["data"]["email_timestream"] = "999999999999999"
    response = client.post('/send-email', 
                         json=test_data,
                         content_type='application/json')
    assert response.status_code == 403
    ...
    test_data["data"]["email_timestream"] = "-123456789"
    response = client.post('/send-email', 
                         json=test_data,
                         content_type='application/json')
    assert response.status_code == 403
